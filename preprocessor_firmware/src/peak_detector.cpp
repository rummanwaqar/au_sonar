...
PeakDetector::PeakDetector(){
    pinMode(GPIO_3,OUTPUT);                                             //pin 7 is being used to monitor the ping status result
    ...
    adc = new ADC();
    adc->setReference(ADC_REFERENCE::REF_EXT, ADC_0);                   //sets external reference for ADC
    adc->setResolution(16);                                             //set the resolution of the ADC in use
    adc->setAveraging(averagingADCValue);                               //sets averaging
    adc->setConversionSpeed(ADC_CONVERSION_SPEED::HIGH_SPEED_16BITS);   //set conversion speed
...
int PeakDetector::getPingStatus(float &peaklevel){
    ...
    float peakinput = adc->analogRead(A8, ADC_0);                   //A8 analogueread(8), 0 to 65535
    peaklevel = (Vref*peakinput)/(resolution);                      //Adjust result, Vref is max
        ...
        if (peaklevel>goodlevel){                                       //Valid ping levels
            if (validcounter==0){                                         //Get ping start time and set validcounter
                offsettime = pingoffset;                                         //set ellaspedMicros back to 0 to avoid rollover
                validcounter = 1;
            }
            if(validcounter){                                           //Offset validping start and end times to remove echoes
                ...
                if(((pingoffset-offsettime) >= pingValidStart) && ((pingoffset-offsettime) < pingValidEnd)){
                    pingvalid = 1;
                    digitalWrite(GPIO_3, HIGH);                         //Ping flag HIGH
                    //Serial.print("Valid");
                    return pingvalid;
                }
                if((pingoffset-offsettime) >= pingValidEnd){                         //Set pingvalid to LOW and clear the validcounter
                    pingvalid=2;
                    digitalWrite(GPIO_3, LOW);                           //Ping flag LOW
                    validcounter = 0;
                    //Serial.print("End");
...
int PeakDetector::getADCAveraging( void ){
    return averagingADCValue;
}

void PeakDetector::setPeakNoise(float halfamp){
    peaknoise = halfamp;
}
