...
class GainControl : public PeakDetector {
 ...
 private:
  ...
  double
      averagePeakLevel;  // Variable to get the average peakLevel of the signal
  ...
  // Returns the time elasped since calling millis on a 'timer'. A simple Helper
  // function
  ...
  // Timers
  unsigned long
      floorGainTimer;  // The timer to track how long gain has been floored
  unsigned long
      invalidPingTimer;  // Timer to track how long gain has been invalid for
  unsigned long nudgeTimer;  // Timer to track when to nudge the gain upwards
  unsigned long
      ledTimer;  // Timer to keep the LED long enough for us to see ping
  ...
  unsigned int invalidPingDuration;  // Time to print debug information after
                                     // invalid ping
  float nudgeGainValue;  // The amount the gain will be nudged upwards after
                         // invalid ping durations
