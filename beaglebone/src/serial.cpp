...
int Serial::write(const std::string& buf) {
  ...
  return serial_port_.write_some(boost::asio::buffer(buf.c_str(), buf.size()),
                                 ec);
}

void Serial::register_receive_callback(
...
void Serial::async_read() {
  ...
  serial_port_.async_read_some(
      boost::asio::buffer(read_buffer_, READ_BUFFER_SIZE_),
      std::bind(&Serial::read_complete, this, std::placeholders::_1,
                std::placeholders::_2));
}
