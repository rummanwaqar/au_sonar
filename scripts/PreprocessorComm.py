    ...
    def run(self):
        while not self.shutdown_flag.is_set():
            ...
            if i >= 0:
                ...
                r = r.decode("utf-8")
                self.buf[0:] = data[i + 1:]
                parsed_type, parsed_data = self.__parse_input(r, self.sonar_params)
                if parsed_type == 'ping':
            ...
            while timeout == 0 or (timeout != 0 and current_timeout > 0):
                if self.write_response is not None:
                    if self.write_response['cmd'] == command and \
                        ...
                        print('wrote {}={}'.format(command, self.write_response['val']))
                        self.write_response = None
                        return True
                    else:
                        self.write_response = None
        ...
        if data is not None:
            ...
            if len(split_string) > 1:
                ...
                if split_string[0][0] == '$':
                    ...
                    if command == 'ping':
                        return 'ping', PreprocessorComm.__parse_ping(split_string[1:])
                    if command == 'set':
                        value = PreprocessorComm.__parse_number(split_string[2])
                        return 'write_response', {'cmd': split_string[1], 'val': value}
                    elif command in command_dict:
                        value = PreprocessorComm.__parse_number(split_string[1])
